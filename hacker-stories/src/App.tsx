...
const getSumComments = (stories: StoriesState) => {
  return stories.data.reduce((result: any, value: any) => result + value.num_comments, 0);
};

const extractSearchTerm = (url: string) =>
  url.substring(url.lastIndexOf('?') + 1, url.lastIndexOf('&')).replace(PARAM_SEARCH, '');
const getLastSearches = (urls: string[]) =>
  Array.from(new Set(urls)).slice(-6, -1).map(extractSearchTerm);
  ...
  return (
    <div className="container">
      ...
      {lastSearches.map((url, index) => (
        <button key={url + index} type="button" onClick={() => handleLastSearch(url)}>
          {url}
        </button>
      ))}
